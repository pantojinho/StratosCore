...
| PR21 | Primary use inside an unpressurized, physically protected aircraft cabin/car/on a desk, with valid barometric altitude/vario through 30,000 ft (~9,144 m); balloon use is secondary | D29 requires pressure-range margin, vent/temperature qualification and pressure/vario bench tests through the limit; D08 BMP581 conflicts, and P27 replacement is not yet accepted |
...
Unpressurized but physically protected aircraft-cabin/car/desk use is primary; valid barometric altitude/vario through the 30,000 ft aircraft reference and 8 h minimum runtime apply to it. Balloon use is secondary and may require additional protection. The pressure sensor sees cabin air through the enclosure vent, so it may approach the outside-air pressure; thermal protection depends on actual installation. The BMP581 has inadequate nominal pressure margin at the limit; [P27](BAROMETER_30K_PROPOSAL.md) is a gated replacement proposal. Actual temperature range, dynamics and a separate balloon acceptance envelope remain unspecified and must be addressed before their corresponding component/environment review closes. Mass and rain/ingress resistance are not design targets for Rev A. Sunlight brightness, ADS-B sensitivity/range and acceptable log loss also remain open. A balloon use case may exceed the pressure sensor's specified range, so out-of-range measurements must be flagged.
