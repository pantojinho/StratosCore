...
Coordination snapshot: `origin/main` at `a5cd4f0` reconciled on 2026-09-22 (GOV-02 vigil retired; SYS-01 #140, ADSB-01 #142, DIG-01 #144, PWR-02 #147, DIG-03 #149 and DIG-02 #154 accepted; DIG-03 enable-polarity correction #152 landed on `main` but its workboard/decision annotation is still pending in open PR #153). The next coordinator must replace this snapshot with the current baseline before assigning work.
...
| DIG-02 | ACCEPTED | 4 | Close microSD power, ESD, card detect, power-fail and enclosure access | MECH-02 for final fit | `DIGITAL_SUPPORT_REVIEW.md`, connector docs | Delivered 2026-09-22 (PR #154): switched `3V3_SD` behind the corrected DIG-03 enable; SPI-mode pin mapping (`SD_CS`->DAT3 chip select); host pull-ups on CMD/DAT0/DAT3 referenced to `3V3_SD` only (no back-power to `3V3_MAIN`); no CLK pull-up; card detect on the `3V3_MAIN` expander input; socket-edge 4-channel ESD array; power-fail fresh-init rule for firmware. Enclosure fit stays with MECH-02/03; insertion-surge value and ESD-array MPN purchase check recorded as TBD |
| DIG-03 | ACCEPTED | 4 | Close USB, expansion, buttons, GPIO expander and all reset/off-state defaults | OWN-02, PWR-02 | connector/control docs, matrix | Delivered 2026-09-22 (PR #149): complete 16-port TCA9535 map with per-net external safe-state resistors; active-low-release enables (dead expander = domains OFF); expander VDD on always-on `3V3_MAIN`; ESP32/RP2040 recovery routes counted without the expander; matrix defaults row closed |
