...
Coordination snapshot: `origin/main` at `bfb9e93` reconciled on 2026-09-22 (GOV-02 vigil retired; SYS-01 #140, ADSB-01 #142, DIG-01 #144, PWR-02 #147 and DIG-03 #149 accepted). The next coordinator must replace this snapshot with the current baseline before assigning work.
...
| DIG-03 | ACCEPTED | 4 | Close USB, expansion, buttons, GPIO expander and all reset/off-state defaults | OWN-02, PWR-02 | connector/control docs, matrix | Delivered 2026-09-22 (PR #149): complete 16-port TCA9535 map with per-net external safe-state resistors; active-low-release convention for every switched-domain enable (dead expander = domains OFF); expander VDD on `3V3_MAIN` (back-power rationale recorded); ESP32/RP2040 recovery routes counted without the expander; electrical-matrix defaults row closed. Per-pin off-state proof remains gate-9/board work; AUD_SW_EN pull-rail-to-`1V8_LOGIC` choice pending DSP-02 confirmation |
