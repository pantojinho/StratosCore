...
The candidate adds one I2C address, one interrupt/status path and a small package. `TPS259474LRPWR` is now the first VBUS eFuse candidate because TI specifies adjustable overvoltage cutoff, circuit-breaker behavior, latch-off, controlled rise time and reverse-current blocking. The named path is `USB4105-GF-A -> signal ESD on D+/D-/CC -> connector-side 1-10 uF VBUS -> TPS259474L -> BQ25887 VBUS`. Exact ILIM, OVLO, dV/dt, ITIMER, thermal behavior, VBUS discharge and USB-compliance limits remain calculations, so this is not a frozen circuit.
