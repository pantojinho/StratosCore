...
MCP6566 ordering-code closure (DS20002143G, Product Identification System): `LT` = 5-lead SC70, `OT` = 5-lead SOT-23, `E` = -40 to +125 °C. Preferred candidate **`MCP6566T-E/LT`** (SC70-5, tape and reel) for the assembled PCBA; **`MCP6566T-E/OT`** (SOT-23-5) is the hand-soldering/rework alternative. Both are the same open-drain single comparator; only the package differs. The open-drain output requires an external pull-up to the RP2040 3.3 V domain (suggested starting point 1-10 kΩ, value to be set by the pulse-timing measurement in the prototype gate below).
