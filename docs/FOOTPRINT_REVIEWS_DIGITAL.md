...
- **Source lineage:** vendored from the upstream KiCad library `Crystal.pretty/Crystal_SMD_3225-4Pin_3.2x2.5mm.kicad_mod` (standard 4-pad 3225 crystal land, generated from the TXC 7M-family drawing). The exact-MPN Abracon source-control drawing #456603 Rev B (2024-09-16, `abracon.com/datasheets/ABM8-272-T3.pdf`) confirms the 3.2 x 2.5 x 0.8 mm package, 4 pads, 50 ohm max ESR, 10 pF CL, 30 ppm tolerance/stability, and Raspberry Pi RP2040/RP235x approval.
- **Manufacturer land-pattern transcription (ABM8 series sheet, machine-read from the rendered drawing):** 4 square pads of 0.8 x 0.8 mm at the corners of a 2 x 2 grid, 1.3 mm horizontal center-to-center between pad columns, 1.05 mm vertical center-to-center between pad rows, 1.0 mm central gap. Terminal sizes on the bottom view: 1.2 mm terminal width, 0.9 +/- 0.15 mm terminal length, 1.0 mm edge distance, 0.65 +/- 0.15 mm inset.
- **Comparison verdict:** the vendored KiCad 3225 footprint uses extended 1.4 x 1.2 mm pads (gull-wing reflow style) instead of the manufacturer's 0.8 x 0.8 mm minimum recommendation — a conservative overlap, not a conflict, but **deciding between manufacturer-minimum and KiCad-extended land is an explicit PENDING choice** for the Astra handoff; record the chosen style in the schematic BOM note.
...
- **Source lineage:** vendored from the upstream KiCad library `Connector_Card.pretty/microSD_HC_Hirose_DM3AT-SF-PEJM5.kicad_mod`. Its embedded description cites Hirose's own 2D drawing for the exact part (download key `DM3AT-SF-PEJM5`, doc_file_id 44099), which is the same controlled drawing family as the Hirose "DRAWING FOR REFERENCE" PDF dated Aug. 1, 2026 retrieved during this review (8 contacts + card-detect switch + 4 shell stakes; 13.85 x 15.95 x 1.68 mm body per the [digital support review](DIGITAL_SUPPORT_REVIEW.md)).
...
Automated image transcription of vector drawing PDFs was attempted on 2026-09-14 and failed (provider timeouts), so upstream KiCad candidates were vendored with their provenance recorded. These files are useful starting points, but KiCad is not the component manufacturer and each residual manufacturer-drawing comparison remains explicit. The sensors/GNSS/RF footprints are covered by [FOOTPRINT_REVIEWS_SENSORS_GNSS.md](FOOTPRINT_REVIEWS_SENSORS_GNSS.md).
