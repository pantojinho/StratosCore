...
| Peripheral reset/enable | Slow non-safety controls may use `TCA9535PWR`; rails/protectors must reach safe states without either MCU | Defaults recorded 2026-09-22 (DIG-03, [control-I/O review](CONTROL_IO_REVIEW.md)): full 16-port map with per-net external pulls; all switched-domain enables active-low-release (dead expander = domains OFF); expander VDD on always-on `3V3_MAIN`; ESP32 and RP2040 recovery routes counted without the expander. Per-pin off-state proof against switched rails remains the board-stage check |
