...
Status: system-level pre-schematic review. Created 2026-09-14; reconciled 2026-09-22 against PWR-04, DIG-01/03, ADSB-01 and DSP-02. A row marked OPEN prevents the affected circuit from being treated as KiCad-ready. An analytical allowance is not a measurement or a complete application circuit.
...
| 3V3_MAIN | `TPS62130ARGTR` candidate from protected 2S | ESP32-S3, RP2040, microSD, sensors, touch, SX1262, rail-control logic and TPS61169 backlight input | ANALYTICAL INVENTORY CLOSED; APPLICATION OPEN | PWR-04 in [power rail plan](POWER_RAIL_PLAN.md) gives ~1,413 mA simultaneous allowance and ~1,767 mA with 25% margin (59% of 3 A rating). Some rows are explicitly DS-typ or ALLOW, not verified maxima. Exact buck application, transient/thermal/EMI/brownout behavior and physical measurements remain open |
...
| `1V8_LOGIC` | `TPS7A2018PDBVR` candidate from `3V3_MAIN` | TFT translator low side and upstream supply for proposed `1V8_AUDIO_SW` load-switch branch | 25 mA ALLOWANCE; AUDIO APPLICATION OPEN | PWR-04 reserves 25 mA for the entire 1.8 V chain, not a measured/display-specified maximum. Preferred reconciliation keeps the TFT on `1V8_LOGIC` and switches only the audio branch; exact CT/QOD/ramp, drop and off-state review remain in [audio-domain review](AUDIO_DOMAIN_REVIEW.md). Confirm Orient supply/current |
...
The proposed main buck's 3 A rating is an upper component capability, not a verified rail requirement. PWR-04's ~1,413 mA model is a labeled simultaneous allowance, not a measured peak; its 25% margin does not close load transients or the 2S source application.
...
| PDM microphone | ESP32 3.3 V clock/data domain; T5838 is 1.8 V | AUD-01 pinned `TXU0202DCUR` directions; preferred net reconciliation puts mic VDD, translator VCCB and OE pull-up on `1V8_AUDIO_SW` | Review exact load-switch application and off-state pins before schematic entry; footprint/acoustic port and RF-noise test remain |
...
| RP2040 boot | W25Q128JVSIQ and ABM8-272-T3 preferred; expose SWD and a reviewed BOOTSEL recovery method | DIG-01 accepted the reset/BOOTSEL/SWD/oscillator circuit on 2026-09-22; exact CAD, corner and physical tests remain |
| Peripheral reset/enable | Slow non-safety controls may use `TCA9535PWR`; rails/protectors must reach safe states without either MCU | Defaults recorded 2026-09-22 (DIG-03, [control-I/O review](CONTROL_IO_REVIEW.md)): full 16-port map with per-net external pulls; `TPS22918` switched-domain enables are **active-high with 100 kohm pull-downs** (dead expander = domains OFF); expander VDD on `3V3_MAIN`; ESP32 and RP2040 recovery routes counted without the expander. Per-pin off-state proof against switched rails remains open |
