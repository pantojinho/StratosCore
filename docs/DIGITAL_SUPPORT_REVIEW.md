...
- **Power-cut write integrity:** W25Q128JV Rev M §8.2.19 warns that data in the page/sector/block under an erase or program operation — including a *suspended* one — may become corrupted on accidental power interruption, and the Reset-command section requires checking BUSY/SUS before any device reset. RP2040 §2.12 provides POR, brown-out detection, supply monitor, external reset and a Source-of-Last-Reset register. Consequence for the off-state review (O05/O06/gate 9): a switched RP2040 domain must guarantee firmware quiesces flash writes (BUSY/SUS clear) before the TPS22918 rail falls — QOD discharge ordering and the host-visible reset reason are the review items, not a resistor value.
