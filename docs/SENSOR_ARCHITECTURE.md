...
| --- | --- | --- | --- |
| ICM-42688-P | Accelerometer + gyro | I2C at 0x68 plus INT1; pin/application entry released; manufacturer-derived footprint passed dimensional second pass and awaits assembler/final-layout review | [TDK DS-000347 v1.9](https://d17t6iyxenbwp1.cloudfront.net/s3fs-public/2026-06/DS-000347%20ICM-42688-P%20v1.9.pdf?VersionId=XL060vOv88u6zsKx9dkBVJEyV452m0eq) |
| MMC5983MA | Three-axis magnetic field and calibrated heading input | I2C at 0x30; polled in Rev A; logical pin carrier entered; manufacturer-derived footprint parses/audits and passed an independent dimensional review; assembler/magnetic placement gates remain | [MEMSIC datasheet Rev A](https://www.memsic.com/Public/Uploads/uploadfile/files/20220119/MMC5983MADatasheetRevA.pdf) |
| BMP581 | Pressure, barometric altitude and variometer input | I2C at 0x46; polled in Rev A; logical pin carrier entered; manufacturer-derived footprint parses/audits and passed an independent dimensional review; assembler/full under-body routing/vent gates remain | [Bosch BST-BMP581-DS004-13](https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmp581-ds004.pdf) |
...
ICM-42688-P DS-000347 v1.9 confirms AD0 low for 0x68, AP_CS tied to VDDIO for I2C, pin 7 tied to ground, and INT2/FSYNC/CLKIN grounded when unused. Its application circuit requires 0.1 µF plus 2.2 µF at VDD and 10 nF at VDDIO. AN-000393 v2.4 says the LGA PCB lands follow the package terminals, recommends clearing solder mask under the body where the board process permits it, gives a 0.1 mm per-pad opening expansion as the fallback, and specifies a 90% stencil-opening-to-land ratio. The project candidate interprets the latter as 90% area while preserving aspect ratio; the assembler must approve the final mask and paste. The similar generic KiCad LGA footprint has larger pads and is not accepted for this device.

The MMC5983MA Rev A page-20 land pattern is now transcribed exactly for copper. Its 1:1 mask/paste is a project process starting point because that datasheet gives no mask, aperture or stencil rule. BMP581 DS004-13 Figure 32 is transcribed with 0.300 x 0.325 mm lands, the 2.200 mm full no-mask opening and a central four-layer routing keepout. Its 1:1 paste is likewise provisional. Both candidates passed independent dimensional comparison but stay out of the schematic footprint assignments until assembler/process approval is recorded; final BMP routing review must also keep unnecessary traces and all vias out from under the complete body.
