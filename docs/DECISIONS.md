...
| P17 | Evaluate `TUSB320LAIRWBR` as the fixed-UFP USB-C CC controller and `TPS259474LRPWR` as the VBUS eFuse; remain at 5 V without USB PD | TI documents default/1.5 A/3 A detection at 0x47. Hardware keeps charger CD disabled until valid cells and an accepted USB current state. PWR-02 (PR #147, merged 2026-09-22) closed the eFuse network (RILM 1.69 kohm -> 1.97 A breaker, UVLO 3.98 V/OVLO 6.50 V dividers, dVdt left open) and the default-current-only USB sink policy from the TUSB320LAI GPIO-mode CC facts; ITIMER sizing, back-power, discharge, thermal and sourcing remain open (O06) |
...
| P19 | Use Hirose `DM3AT-SF-PEJM5` as the preferred microSD socket candidate on shared SPI | Exact drawing `0000947170 / EDC-325165-00-00` Rev 4, card detect, lands and operational card envelope reviewed; assembler process, enclosure access, ESD/power switching and factory sourcing remain gates |
...
| P24 | Use GCT `USB4105-GF-A`, TI `TPD4E05U06DQARG4`, JST `BM12B-GHS-TBT`/`GHR-12V-S`, Alps `SKSCLCE010` buttons and TI `TCA9535PWR` slow I/O | Preferred support candidates only; VBUS protection remains open. DIG-03 (PR #149, merged 2026-09-22) closed the TCA9535 reset/off-state defaults: full 16-port map with per-net external safe-state resistors, active-low-release convention for every switched-domain enable (dead expander = domains OFF), expander VDD on always-on `3V3_MAIN`, and ESP32/RP2040 recovery routes counted without the expander. Every footprint, cable, current limit, hot-plug/back-power and enclosure fit, plus the per-pin off-state proof against switched rails, still must be independently reviewed |
