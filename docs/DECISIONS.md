...
| P17 | Evaluate `TUSB320LAIRWBR` as the fixed-UFP USB-C CC controller and `TPS259474LRPWR` as the VBUS eFuse; remain at 5 V without USB PD | TI documents default/1.5 A/3 A detection at 0x47. Hardware keeps charger CD disabled until valid cells and an accepted USB current state. PWR-02 (PR #147, merged 2026-09-22) fixed the eFuse network (RILM/UVLO/OVLO dividers, dVdt open) and the default-current-only enumeration policy from the GPIO-mode CC facts; ITIMER sizing, back-power, discharge, thermal and sourcing remain open (O06) |
