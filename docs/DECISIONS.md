...
### 2026-09-14: Sol-class closure session (Hermes branch hermes/pre-astra-closure)

- **Owner:** standing main authorization; no locked decision changed.
- **Work recorded:** power budget re-derived from exact datasheets (FLIGHT 1.684 W; two-cell 12.31 h estimate; O13 arithmetic closed in [power budget](POWER_BUDGET.md)); RP2040 UART host framing contract implemented with 17 passing tests (O12 host stage closed in [open questions](OPEN_QUESTIONS.md)); electrical matrix evidence rows closed with exact regulator/interface figures including the TUSB320LAI back-power disposition and the calculated 974 mA 3V3_MAIN peak ([matrix](ELECTRICAL_COMPATIBILITY_MATRIX.md)); footprints vendored/built for W25Q128JVSIQ, ABM8-272-T3, DM3AT-SF-PEJM5, U.FL-R-SMT-1(60) and TPD1E0B04DPYR with provenance ([digital reviews](FOOTPRINT_REVIEWS_DIGITAL.md), [sensors/GNSS reviews](FOOTPRINT_REVIEWS_SENSORS_GNSS.md)).
- **Impact:** reduces the remaining pre-Astra queue to bounded transcription and review tasks (ICM-42688-P and MAX-M10S Figure 30 transcriptions, ABM8/DM3 drawing comparisons, BQ25887 3.3 V pull-up threshold confirmation, LoRa/ADS-B RF reference freeze) plus the externally blocked gates (samples, qualified battery reviewer, factory stack, prototypes).
- **Outcome:** no decision status changed from LOCKED/OPEN; evidence depth advanced. All added numbers carry datasheet/drawing citations; unverified geometry is recorded as PENDING, not claimed.
