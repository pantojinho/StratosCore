...
Manufacturer names for selected parts follow the primary source index. Supply ranges and packages are left TBD where a full electrical/package audit has not been completed. `Preferred Supplier` and `JLCPCB/LCSC Part Number` are TBD throughout: no approved supplier or verified stock identifier exists yet. Assembly entries are targets, not a confirmation that a particular factory accepts the part.

Sources: [locked decisions](../docs/DECISIONS.md), [manufacturer index](../hardware/datasheets/README.md), and [power candidates](../docs/POWER_ARCHITECTURE_OPTIONS.md). Future complete BOMs need quantities, actual reference designators and sourced ordering suffixes, without modifying this requested preliminary CSV schema casually.
