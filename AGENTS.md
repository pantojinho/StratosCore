...
17. The current priority is hardware and PCBA readiness. Astra has assumed full technical leadership, coordination, review and integration of the project, including engineering closure and KiCad candidate refinement (owner accepted 2026-09-24; see `docs/ASTRA_PROJECT_CONTROL.md`). Claude, Hermes, Sol and other AI agents execute bounded tasks delegated by Astra and return evidence for review. Final baseline implementation and manufacturing remain subject to their existing gates. Limit firmware work to evidence or interface fixtures required to validate the hardware until the hardware baseline is ready.
18. During multi-agent swarm work, follow `docs/SWARM_PROTOCOL.md`. Workers use isolated `codex/<task-id>-...` branches, own only their assigned files, open reviewable PRs and never merge their own work. Astra is the coordinator and integrator; only Astra updates shared status/decision/open-question/handoff/workboard files or publishes the reviewed integration to `main`. No-change audits do not create commits.

## Verification for the current phase

Check required files/directories, relative Markdown links, BOM columns and locked-part coverage, official license text provenance, power arithmetic, decision consistency, host ADS-B tests, KiCad hierarchy ERC and `git diff --check`. Schematic circuits and final baseline placement/routing remain gated by exact component evidence and accepted owner decisions. Bounded refinement of the existing comparison candidate follows `docs/ASTRA_CANDIDATE_REFINEMENT_PROMPT.md`.
